...
{
    INFO("EventLoop created %p in thread %d\n", this, threadID_);
    if (t_loopInThisThread)
    {
        FATAL("Another EventLoop %p exits in this thread %d\n", t_loopInThisThread, threadID_);
    }
    else
    {
        t_loopInThisThread = this;
    }
    wakeupChannel_->setReadCallback(std::bind(&EventLoop::handleRead, this)); // 设置wakeup的事件类型以及发生事件后的回调操作
    wakeupChannel_->enableReading();                                          // 每一个EventLoop都监听wakeupChannel_的EPOLLIN事件了
}
EventLoop::~EventLoop()
{
    wakeupChannel_->disableAll(); // 给wakeupChannel移除所有的感兴趣事件
    wakeupChannel_->remove();     // 把wakeupChannel从EventLoop上删除掉
    ::close(wakeupFd_);
...
void EventLoop::handleRead()
{
    ...
    if (n != sizeof(one))
    {
        ERROR("EventLoop::handleRead() read %lu bytes instead of 8\n", n);
    }
}
...
void EventLoop::loop()
{
    ...
    INFO("EventLoop %p start looping \n", this);
    ...
    while (!quit_)
    {
        ...
        {
            // Poller监听器监听哪些channel发生了事件然后上报给EventLoop,通知Channel处理相应的事件
...
void EventLoop::runInLoop(Functor cb)
{
    ...
    {
        // 在非当前EventLoop中执行回调，就需要唤醒EventLoop所在的线程执行cb
        queueInLoop(cb);
    }
}
...
void EventLoop::wakeup()
{
    ...
    if (n != sizeof(one))
    {
        ERROR("EventLoop::wakeup() write %lu instead of 8\n", n);
    }
}
