...
void Socket::shutdownWrite()
{
    ...
    {
        ERROR("shutdownWrite error");
    }
}

void Socket::setTcpNoDelay(bool on)
{
    //TCP_NODELAY 用于禁用Nagle 算法
    //Nagel 算法用于减少网络上传输的小数据包数量
    //将TCP_NODELAY 设置为1，可以禁用该算法，允许小数据包立即发送
    int optval = on ? 1 : 0;
